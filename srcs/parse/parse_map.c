...
/*   Updated: 2025/06/24 00:41:58 by hnakayam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	check_map_size(t_parse_data *data, t_vars *vars)
{
	...
	if (data->max_width > MAX_MAP_WIDTH)
	{
		cleanup_raw_map(data->raw_map, data->height);
		error_message_and_free(vars,
			ft_strdup("Map size exceeds maximum allowed"), 1);
...
void	handle_map_line(t_vars *vars, char *line, t_parse_data *data)
{
	if (ft_strlen(line) > 0)
	{
...
void	process_map_line(t_vars *vars, char **line, t_parse_data *data)
{
	...
	if (ft_strlen(*line) == 0 || is_valid_map_line(*line))
	{
		...
		if (ft_strlen(*line) > 0)
			handle_map_line(vars, *line, data);
	}
	else
	{
		cleanup_raw_map(data->raw_map, data->height);
